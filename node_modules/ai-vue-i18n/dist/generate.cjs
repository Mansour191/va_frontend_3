...
var FormData$1 = /*@__PURE__*/scriptLoader.getDefaultExportFromCjs(form_dataExports);
...
var proxyFromEnv = /*@__PURE__*/scriptLoader.getDefaultExportFromCjs(proxyFromEnvExports);
	...
	debug_1 = function () {
	  if (!debug) {
	    try {
	      ...
	      debug = scriptLoader.requireSrc()("follow-redirects");
	    }
	    catch (error) { /* */ }
...
var followRedirects = /*@__PURE__*/scriptLoader.getDefaultExportFromCjs(followRedirectsExports);
...
var httpAdapter = isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    const emitter = new require$$0$2.EventEmitter();
...
const getLanguagesFiles = async (options) => {
    ...
    for (const { fileName } of options.translateList) {
        ...
        try {
            const { exportType, content: translated } = await scriptLoader.fileOperator.getFileContent(filePath) || {};
    ...
    scriptLoader.fileOperator.setTotalTranslateMessages(languagesFileContent, options.clearInexistence);
    return languagesFileContent;
};
// 为每个键收集需要翻译的语言
async function getkeyLanguageMap(options, check = false) {
    const messages = scriptLoader.fileOperator.messages;
    let useCache = false;
    ...
    const translationLog = scriptLoader.fileOperator.loadTranslationCache(options.outputDir);
    ...
    for (const key of Object.keys(messages)) {
        ...
        for (const { fileName } of translateList) {
            ...
            if (!translated?.[key]) {
                ...
                if (cacheValue) {
                    useCache = true;
                    scriptLoader.fileOperator.updateTranslateMessages(fileName, key, cacheValue);
                    continue;
                }
        ...
        Promise.resolve(analyzePromise).then((list) => {
            for (const item of list) {
                ...
                results.forEach((text, index) => {
                    ...
                    scriptLoader.fileOperator.updateTranslateMessages(lang, id, text);
                    scriptLoader.fileOperator.updateTranslationCache(id, lang, text);
                });
            }
            resolve(list);
...
const executeTranslate = async (options) => {
    try {
        ...
        const executeRequest = scriptLoader.requestPool(Math.max(1, options.parallerSize));
        const promiseList = [];
        ...
        scriptLoader.fileOperator.saveLanguageFiles(options.outputDir, options.exportType);
        scriptLoader.fileOperator.saveTranslationCache(options.outputDir);
...
const generateVueFile = (path, options) => {
    try {
        ...
        const { template, script, scriptSetup } = scriptLoader.getVueModule(code, options.vueVersion);
        if (template) {
            const templateLoader = new scriptLoader.TemplateLoader(options, path);
            templateLoader.excute(template);
        }
        if (script) {
            const scriptLoader$1 = new scriptLoader.ScriptLoader(options, path);
            scriptLoader$1.excute(script);
        }
        if (scriptSetup) {
            const scriptLoader$1 = new scriptLoader.ScriptLoader(options, path);
            scriptLoader$1.excute(scriptSetup, true);
        }
    }
    catch (error) {
...
const generateI18nFiles = async (options, writeMessage = true) => {
    ...
    await scriptLoader.fileOperator.initMessage(localeFile, options.clearInexistence);
    const files = scriptLoader.fileOperator.getAllFiles(options);
    for (let file of files) {
        ...
        if (path.extname(file) === '.vue') {
            ...
            const scriptLoader$1 = new scriptLoader.ScriptLoader(options, file);
            scriptLoader$1.excute(code);
        }
    }
    if (!writeMessage) {
        return;
    }
    await scriptLoader.fileOperator.writeMessages(outputDir, localeFile, options.exportType, scriptLoader.fileOperator.messages);
...
const generate = async () => {
    try {
        const options = await scriptLoader.fileOperator.getConfig(true);
